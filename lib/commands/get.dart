...
List<Game> _parseGames(data) {
  ...
  return games.map<Game>((e) => Game.fromMap(e)).where((e) => e.status == "ACTIVE").toList();
}
...
void _displayGameList(List<Game> games, ArgResults? argResults, CliConfig config) {
  final List<MapEntry<Promotion?, dynamic>> discountedGames = [];
  final List<List<String>> otherGames = [];
